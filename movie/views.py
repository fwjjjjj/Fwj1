...
def moviehome(request):
    # return HttpResponse('<h1>欢迎来到movie应用<h1>')
...
def home(request):
    # return HttpResponse('<h1>hello欢迎来到我的首页</h1>')
    return render(request, 'home.html', {'name': "HCM"})

def signup(request):
    email=request.GET.get('email')
    return render(request,'sigunp.html',{'email':email})
@login_required
